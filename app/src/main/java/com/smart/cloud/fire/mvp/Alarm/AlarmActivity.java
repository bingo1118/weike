...
public class AlarmActivity extends MvpActivity<AlarmPresenter> implements AlarmView {
    ...
    TextView stop_alarm;
    @Bind(R.id.deal_alarm_btn)
    Button deal_alarm_btn;
     ...
     */
    private void init() {
        if(mPushAlarmMsg!=null&&(mPushAlarmMsg.getDeviceType()==18||mPushAlarmMsg.getDeviceType()==19)){
...
//        }
            ...
            //取消报警
            RxView.clicks(deal_alarm_btn).throttleFirst(2, TimeUnit.SECONDS).subscribe(new Action1<Void>() {
                @Override
                public void call(Void aVoid) {
                    String userId = SharedPreferencesManager.getInstance().getData(mContext,
                            SharedPreferencesManager.SP_FILE_GWELL,
                            SharedPreferencesManager.KEY_RECENTNAME);
                    presenter.dealAlarm(userId, mPushAlarmMsg.getMac());//@@5.19添加index位置参数
                }
            });
        }
